...
class GameViewController: UIViewController, /*FBSDKLoginButtonDelegate,*/ FBSDKAppInviteDialogDelegate {
  ...
  func saveScoreToFirebaseNotification(_ notification: Notification) {
    ...
    saveFacebookUserInfoToDB(UserDefaults.standard.string(forKey: kFirebaseUID)!)
  }
  ...
  func logoutOfFacebook(_ completion: (_ result: Bool) -> Void) {
    ...
    if let _ = UserDefaults.standard.string(forKey: kFirebaseUID) {
      print("REBELOPER: Deleting existing UserDefaults data for key \(kFirebaseUID)")
      UserDefaults.standard.set(nil, forKey: kFirebaseUID)
      UserDefaults.standard.synchronize()
    } else {
      print("REBELOPER: No data found in UserDefaults for key \(kFirebaseUID). No need to delete.")
    }
  ...
  func completeSignIn(_ uid: String) {
    ...
    UserDefaults.standard.set(uid, forKey: kFirebaseUID)
    UserDefaults.standard.synchronize()
    print("REBELOPER: Data saved to UserDefaults = \(UserDefaults.standard.string(forKey: kFirebaseUID))")
    loginView.isHidden = true
  ...
  func isUserSignedIn() -> Bool {
    //if let _ = KeychainWrapper.defaultKeychainWrapper().stringForKey(kFirebaseUID) {
    if UserDefaults.standard.string(forKey: kFirebaseUID) != nil {
      return true
    } else {
      return false
    }
